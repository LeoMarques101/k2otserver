...
local config = {
    attackRadius = {x = 7, y = 5, targetDistance = 4, walkDistance = 7},
...
local function rangedAttack(npcId, targetId)
    ...
    if npc and creature then
        doTargetCombat(npcId, targetId, COMBAT_PHYSICALDAMAGE, -config.rangedDamageValue.min, -config.rangedDamageValue.max, CONST_ME_NONE)
                ...
                if npcPosition:getDistance(targetPosition) <= 1 then
                    if pvpNpcs[npcId].melee == true then
                        if math.random(1,10) > 2 then -- 80% chance to hit
                            -- Melee Hit
                            doTargetCombat(npcId, targetId, COMBAT_PHYSICALDAMAGE, -config.meleeDamageValue.min, -config.meleeDamageValue.max, CONST_ME_NONE)
                            -- doTargetCombat(npcId, targetId, COMBAT_PHYSICALDAMAGE, -math.floor(10 / 100 * target:getMaxHealth()), -math.floor(25 / 100 * target:getMaxHealth()), CONST_ME_NONE)
                        else -- missed
