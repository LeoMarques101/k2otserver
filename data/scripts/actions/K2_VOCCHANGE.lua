...
function K2_ALAV_VOCCHANGE.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    elseif custo > player:getMoney() then
    	...
    	local query_k2vocchange ={
    		...
    		"WHERE `player_id` = ".. playerid .. " and `vocation` = ".. VOCACOES_query[voc_atual:getId()].id,
    	}
	    ...
	    local Info = db.storeQuery("SELECT * FROM `players_k2vocchange` WHERE `player_id` = ".. playerid .. " and `vocation` = ".. VOCACOES_query[voc_desejada:getId()].id)
        if Info ~= false then
...
function K2_ALAV_VOCCHANGE_talk.onSay(player, words, param)
		...
		local voc_query = {
			...
			[2] = db.storeQuery("SELECT * FROM `players_k2vocchange` WHERE `player_id` = ".. playerid .. " and `vocation` = ".. 3),
			[3] = db.storeQuery("SELECT * FROM `players_k2vocchange` WHERE `player_id` = ".. playerid .. " and `vocation` = ".. 4),
		...
		for i=1,3 do
			...
			if VOCACOES_query[voc_atual:getId()] == i then
		    	...
		    	txt = txt .. "\n\n---> ".. VOCACOES_query_name[i] .." [Voc. atual]"
